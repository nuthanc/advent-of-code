...
file_path = path.join(THIS_DIR, 'input2.txt')
...
# My solution doesn't consider the container bags, it only considers the contained bags
def find_bags(bags):
    if len(bags) == 0:
        return 1
    ...
    for num, bag in bags:
        ...
        n = int(num)
        part_sum += n * find_bags(bag_dict[bag])
