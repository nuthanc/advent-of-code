...
def dfs(row, col, heightmap, directions, seen, size):
    ...
    for direction in directions:
        ...
        within_bounds = neighbor_row >= 0 and neighbor_row < len(
            heightmap) and neighbor_col >= 0 and neighbor_col < len(heightmap[0])
        ...
        if within_bounds and not_seen and heightmap[neighbor_row][neighbor_col] != '9':
            ...
            if neighbor > current:
                size += dfs(neighbor_row, neighbor_col,
                            heightmap, directions, seen, 1)
    return size
